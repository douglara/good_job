...
require_relative '../spec/test_app/app/jobs/example_job'
...
def make_seed(jobs, queues)
  ...
  queues.times.each do | queue_count |
    jobs_data = Array.new(jobs) do |i|
      ExampleJob.set(queue: "queue_#{queue_count + 1}").perform_later(:SUCCESS_TYPE)
    end
  end
end
...
def run_default_order_until_last_queue
  ...
  while current_queue_name != "queue_#{@queues}" do
    GoodJob::Execution.unfinished.priority_ordered.only_scheduled.limit(1).with_advisory_lock(unlock_session: true) do |good_jobs|
...
def run_queue_random_order_until_last_queue
  current_queue_name = ""
  while current_queue_name != "queue_#{@queues}" do
    GoodJob::Execution.unfinished.priority_ordered_randomized_queues.only_scheduled.limit(1).with_advisory_lock(unlock_session: true) do |good_jobs|
      current_queue_name = good_jobs.first.queue_name
      good_jobs.first&.destroy!
    end
  end
